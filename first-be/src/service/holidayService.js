...
const getHolidaysForCountry = async (
  countryCode,
  isUkraine,
  year = moment().year(),
  filterWeekend = false,
...
const getHolidaysForOnyDayNotification = async () => {
  const today = moment()
  const { talents, talentsForToday } = await getUpcomingBirthdaysIn1Days()
  const endDayForSearch = today.clone().add(1, 'days').format('DD')

  return {
    talents: talents,
    talentsForToday: talentsForToday,
    monthName: today.format('MMMM'),
    dayNumber: endDayForSearch
  }
}
  ...
  return await Talent.findAll({
    where: {
      [Op.or]: daysToCheck.map(date => ({
        [Op.and]: [
          Sequelize.where(Sequelize.fn('MONTH', Sequelize.col('birthday')), date.month),
          Sequelize.where(Sequelize.fn('DAY', Sequelize.col('birthday')), date.day)
        ]
      }))
    },
    include: [
...
const getUpcomingBirthdaysIn1Month = async () => {
  ...
  if (startMMDD <= endMMDD) {
    dateCondition = {
      [Op.and]: [
        Sequelize.where(Sequelize.fn('DATE_FORMAT', Sequelize.col('birthday'), '%m-%d'), {
    ...
    dateCondition = {
      [Op.or]: [
        Sequelize.where(Sequelize.fn('DATE_FORMAT', Sequelize.col('birthday'), '%m-%d'), { [Op.gte]: startMMDD }),
        Sequelize.where(Sequelize.fn('DATE_FORMAT', Sequelize.col('birthday'), '%m-%d'), { [Op.lte]: endMMDD })
      ]
    }
  }
...
const getUpcomingBirthdaysIn1Days = async () => {
  const today = moment().utc()
  let targetDate = today.clone().add(1, 'days')

  let targetMonth = targetDate.month() + 1 // month() is 0-indexed
  ...
  const talents = await Talent.findAll({
    where: {
      [Op.and]: [
        { inactive: false },
        Sequelize.where(Sequelize.fn('MONTH', Sequelize.col('birthday')), targetMonth),
        Sequelize.where(Sequelize.fn('DAY', Sequelize.col('birthday')), targetDay),
        {
  ...
  targetMonth = targetDate.month() + 1 // month() is 0-indexed
  targetDay = targetDate.date()

  const talentsForToday = await Talent.findAll({
    where: {
      [Op.and]: [
        { inactive: false },
        Sequelize.where(Sequelize.fn('MONTH', Sequelize.col('birthday')), targetMonth),
  ...
  return { talents, talentsForToday }
}
...
const getUpcomingBirthdaysCustomersIn1Days = async () => {
  ...
  return await Customer.findAll({
    where: {
      [Op.and]: [
        { inactive: false },
        Sequelize.where(Sequelize.fn('DAYOFYEAR', Sequelize.col('birthday')), {
          [Op.between]: [today.dayOfYear(), targetDate.dayOfYear()]
        })
      ]
    },
    order: [[Sequelize.fn('DAYOFYEAR', Sequelize.col('birthday')), 'ASC']]
  })
}

const getUpcomingAnniversaries = async (type = '') => {
  const today = moment()
  let endDate, startMMDD, endMMDD

  if (type != 'tomorrow') {
    endDate = moment().add(type === '' ? 3 : 0, 'days')

    startMMDD = today.format('MM-DD')
    endMMDD = endDate.format('MM-DD')
  } else {
    endDate = moment().add(1, 'days')

    startMMDD = endDate.format('MM-DD')
    endMMDD = endDate.format('MM-DD')
  }
  ...
  if (startMMDD <= endMMDD) {
    ...
    dateCondition = {
      [Op.or]: [
        Sequelize.where(Sequelize.fn('DATE_FORMAT', Sequelize.col('startDate'), '%m-%d'), { [Op.gte]: startMMDD }),
        Sequelize.where(Sequelize.fn('DATE_FORMAT', Sequelize.col('startDate'), '%m-%d'), { [Op.lte]: endMMDD })
      ]
    }
  }
