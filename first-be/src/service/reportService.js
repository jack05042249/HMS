...
const getReport = async payload => {
  ...
  if ((customers && customers.length) || (agencies && agencies.length)) {
    whereConditions.inactive = false
  }
  ...
  if (type === 'monthly') {
    ...
    for (const talent of talents) {
      ...
      const totalUsed = await getTalentAllUsedDays(talent.id, talent.workFromMonday)
