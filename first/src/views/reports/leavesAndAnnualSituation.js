...
const LeavesAndAnnualSituation = ({ type }, ref) => {
  ...
  const filteredOrgz = filter ? organizations.filter(orgzs => {
    const { name } = orgzs
    if (name.toLocaleLowerCase().includes(filter.toLocaleLowerCase()) || name.toLocaleLowerCase().includes(filter.toLocaleLowerCase())) {
      return orgzs
    } return null

  }) : organizations

  const filteredTalents = filter ? aggregatedTalents.filter(employee => {
    const { fullName } = employee
    if (fullName.toLocaleLowerCase().includes(filter.toLocaleLowerCase()) || fullName.toLocaleLowerCase().includes(filter.toLocaleLowerCase())) {
      return employee
    } return null
  ...
  function handleFetchDataClick() {
    ...
    if (agenciesState.length && !organizationsState.length && !talentState.length){
      setSelectedFilter(ReportsFilters.AGENCY)
    }
    ...
    if (talentState.length && !organizationsState.length && !agenciesState.length) {
      setSelectedFilter(ReportsFilters.TALENT)
    }
  }
  ...
  const toggleShowReports = useCallback((type) => {
    setLoading(true);
    setShouldFetchData(true);
    if (type === 'monthly') {
      clearFilterDatepickerData();
    ...
    <>
      <div className="flex mb-5 justify-between px-[20px]">
        <div className="border b-[#E7E7E7] max-h-[40px] py-[8px] px-[16px] rounded-md w-[350px] flex items-center">
          ...
          <span> <icons.calendarDotes style={{ width: '18px', height: '18px' }} /> </span>
          <DatePicker
            selected={startDate}
            onChange={onChange}
        ...
        <div className="flex flex-col font-medium text-[12px]">
          <div className="flex gap-2 items-center relative">
            ...
            <div className="relative">
              <button onClick={() => toggleFilters(ReportsFilters.ORGANIZATIONS)}
                      className={`flex gap-1 ${selectedFilter === ReportsFilters.ORGANIZATIONS ? 'bg-[#4D4AEA0D] rounded-md py-2 px-3' : 'py-2 px-3'}`}>
                <span> <icons.customerDefault style={{ width: '16px', height: '16px' }} /> </span>Customer
              </button>
              {
                showCustomerPopup && (
                  <div className="w-[225px] px-5 py-2 absolute left-0 bg-[#FFFFFF] shadow-xl z-40 rounded-md"
                    ...
                    <div className="overflow-y-scroll max-h-[150px]">
                      <div className="pl-2 my-2 flex items-center">
                        ...
                        />
                        <label htmlFor="selectAllAgencies" className="text-[14px] whitespace-nowrap underline">Select
                          all</label>
                      </div>
                        ...
                        return (
                          <div key={id} className="flex flex-col pl-2 my-2">
                            <div className="flex">
                              ...
                              />
                              <label htmlFor={id} className="ml-10 text-[14px]">{name}</label>
                            </div>
                          </div>
                        );
                      })}
            ...
            <div className="relative">
              <button onClick={() => toggleFilters(ReportsFilters.AGENCY)}
                      className={`flex gap-1 ${selectedFilter === ReportsFilters.AGENCY ? 'bg-[#4D4AEA0D] rounded-md py-2 px-3' : 'py-2 px-3'}`}>
                <span> <icons.orgz style={{ width: '16px', height: '16px' }} /> </span> Agency
              </button>
              {
                showAgencyPopup && (
                  <div className="w-[225px] px-5 py-2 absolute left-0 bg-[#FFFFFF] shadow-xl z-40 rounded-md"
                        ...
                        return (
                          <div key={id} className="flex flex-col pl-2 my-2 overflow-x-scroll">
                            <div className="flex">
                              ...
                              />
                              <label htmlFor={id} className="text-[14px] whitespace-nowrap">{name}</label>
                            </div>
                          </div>
                        );
                      })}
            ...
            <div className="relative">
              <button onClick={() => toggleFilters(ReportsFilters.TALENT)}
                      className={`flex gap-1 ${selectedFilter === ReportsFilters.TALENT ? 'bg-[#4D4AEA0D] rounded-md py-2 px-3' : 'py-2 px-3'}`}>
                <span> <icons.userDark style={{ width: '16px', height: '16px' }} /> </span>By Employee
              </button>
              {
                showEmployeePopup && (
                  <div className="w-[225px] px-5 py-2 absolute left-0 bg-[#FFFFFF] shadow-xl z-40 rounded-md"
                        ...
                        return (
                          <div key={id} className="flex flex-col pl-2 my-2">
                            <div className="flex">
                              ...
                              />
                              <label htmlFor={id} className="ml-10 text-[14px]">{fullName}</label>
                            </div>
                          </div>
                        );
                      })}
          ...
          {reportRecords.length && (selectedFilter === ReportsFilters.AGENCY || selectedFilter === ReportsFilters.ORGANIZATIONS) ? (
            <div className="flex justify-start mt-2.5 items-center overflow-x-scroll">
              {agenciesState.length && (selectedFilter === ReportsFilters.AGENCY) ?  (
                <div className="flex whitespace-nowrap">
                  Filtered by Agency: <span className="mx-2"> <icons.orgz
                  ...
                  <div className="flex items-center max-w-[100px] whitespace-nowrap">
                    {getSelectedAgenciesNames().map((agency, index) => (
                      <div key={index} className="border border-gray-400 mr-2 mb-1 px-2 rounded-md drop-shadow">
                        <span>{agency}</span>
                        <span className="text-blue-500">({reportRecords.filter(record => record.agency.name === agency).length})</span>
                      </div>
                    ))}
                  </div>
                </div>
              ): null}
              {organizationsState.length && (selectedFilter === ReportsFilters.ORGANIZATIONS) ? (
                <div className="flex whitespace-nowrap">
                Filtered by Customers: <span className="mx-2"> <icons.customerDefault
                  style={{ width: '16px', height: '16px' }} /> </span>
                  <div className="flex items-center  max-w-[100px] whitespace-nowrap">
                  {getSelectedCustomersNames().map((customer, index) => (
                    <div key={index} className="border border-gray-400 mr-2 mb-1 px-2 rounded-md drop-shadow">
                      <span>{customer}</span>
                      <span className="text-blue-500">({reportRecords.filter(record => record.customers.some(c => c.name === customer)).length})</span>
                    </div>
                  ))}
                  </div>
                </div>
        ...
        <div>
          <button
            ...
            <span> <icons.downloadIcon /> </span> Download excel
          </button>
        </div>
      </div>
